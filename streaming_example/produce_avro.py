...
from publish import publish_to_kafka_avro, publish_to_kafka, publish_avro
...
def create_product_list():
    ...
    for p in csv_products:
        ...
        publish_to_kafka_avro(topic_products, new_product)
        ...
        for y in range(0, random_transaction_item_quantity()):
            ...
            if rnd_propensity_to_buy == previous_rnd_propensity_to_buy:
                rnd_propensity_to_buy = closest_product_match(
                    propensity_to_buy_range, random.randint(
                        range_min, range_max)
                )
            ...
            for p in products:
                if p.propensity_to_buy == rnd_propensity_to_buy:
                    ...
                    publish_to_kafka_avro(topic_purchases, new_purchase)
...
def restock_item(product_id):
    for p in products:
        if p.product_id == product_id:
            ...
            publish_to_kafka_avro(topic_inventories, new_inventory)
            break
